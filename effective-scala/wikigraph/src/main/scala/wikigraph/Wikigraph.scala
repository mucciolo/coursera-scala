...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client
      .linksFrom(of)
      .flatMap(ids => WikiResult.traverse(ids.toSeq)(client.nameOfArticle).map(_.toSet))
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      q.dequeueOption match {
        ...
        case Some(((currentDepth, _), _)) if currentDepth >= maxDepth =>
            WikiResult.successful(None)

        case Some(((currentDepth, currentArticle), remainingQueue)) =>
          client.linksFrom(currentArticle)
            .flatMap { articleIds =>
              if articleIds.contains(target) then
                WikiResult.successful(Some(currentDepth))
              else
                val unvisitedIds = articleIds.diff(visited)
                val nextQueue = remainingQueue.enqueueAll(
                  unvisitedIds.map(articleId => (currentDepth + 1, articleId))
                )

                iter(visited ++ unvisitedIds, nextQueue)
            }
  ...
  /**
    ...
    */
  def distanceMatrix(
    titles: List[String], maxDepth: Int = 50
  ): WikiResult[Seq[(String, String, Option[Int])]] =
    val allArticlesPairs = for {
      sourceTitle <- titles
      distTitle <- titles if sourceTitle != distTitle
    } yield (sourceTitle, distTitle)

    WikiResult.traverse(allArticlesPairs) {
      case (sourceTitle, distTitle) =>
        for {
          sourceId <- client.searchId(sourceTitle)
          distId <- client.searchId(distTitle)
          distance <- breadthFirstSearch(sourceId, distId, maxDepth)
        } yield (sourceTitle, distTitle, distance)
    }
