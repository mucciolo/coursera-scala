...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown <= 0 then
      ...
      val nextPosition = Motion.movePoint(position, direction, propulsionSpeed)
      Launched(countDown - 1, nextPosition, direction, numberOfParticles, particlesColor)
    }
...
case class Particle(horizontalSpeed: Double, verticalSpeed: Double, position: Point, color: Color):
  ...
  def next: Particle =
    ...
    // air friction
    val updatedVerticalSpeed: Double = Motion.drag(verticalSpeed - gravity)
