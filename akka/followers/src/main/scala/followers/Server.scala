...
object Server extends ServerModuleInterface {
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      ...
      var nextSequenceNr: Int = 1

      {
        case event if event.sequenceNr == nextSequenceNr && buffer.isEmpty =>
          nextSequenceNr += 1
          List(event)

        case event if event.sequenceNr == nextSequenceNr && buffer.nonEmpty =>
          val identity = (List.empty[Event], List.empty[Event])
          val (resultDesc, updatedBufferDesc) = (event :: buffer).sortBy(_.sequenceNr).foldLeft(identity) {
            case ((Nil, Nil), z) =>
              (z :: Nil, Nil)
            case ((xs, ys), z) if xs.head.sequenceNr + 1 == z.sequenceNr =>
              (z :: xs, ys)
            case ((xs, ys), z) =>
              (xs, z :: ys)
          }

          nextSequenceNr = resultDesc.head.sequenceNr + 1
