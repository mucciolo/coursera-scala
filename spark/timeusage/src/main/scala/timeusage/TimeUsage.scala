...
object TimeUsage extends TimeUsageInterface {
  ...
  val leisurePrefixes: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09", "t10",
                                           "t12", "t13", "t14", "t15", "t16", "t18")

  def startWithAnyOf(x: String, prefixes: Iterable[String]): Boolean = prefixes.exists(x.startsWith)

  def hasPrimaryPrefix: String => Boolean = startWithAnyOf(_, primaryPrefixes)

  def hasWorkingPrefix: String => Boolean = startWithAnyOf(_, workingPrefixes)

  def hasLeisurePrefix: String => Boolean = startWithAnyOf(_, leisurePrefixes)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    @tailrec
    def iterate(columnNames: List[String],
                primary: List[Column],
                working: List[Column],
                leisure: List[Column]): (List[Column], List[Column], List[Column]) = columnNames match {
      case Nil =>
        (primary, working, leisure)

      case name :: tail =>
        if (hasPrimaryPrefix(name)) iterate(tail, col(name) :: primary, working, leisure)
        else if (hasWorkingPrefix(name)) iterate(tail, primary, col(name) :: working, leisure)
        else if (hasLeisurePrefix(name)) iterate(tail, primary, working, col(name) :: leisure)
        else iterate(tail, primary, working, leisure)
    }

    iterate(columnNames, List.empty, List.empty, List.empty)
  }
  ...
  def timeUsageSummary(primaryNeedsColumns: List[Column],
    ...
    primaryNeedsColumns.aggregate(lit(0))((sum, col) => sum + df(col.toString()), _ + _)
                       .divide(60d)
                       .as("primaryNeeds")

    val workProjection: Column =
      workColumns.aggregate(lit(0))((sum, col) => sum + df(col.toString()), _ + _)
                 .divide(60d)
                 .as("work")

    val otherProjection: Column =
      otherColumns.aggregate(lit(0))((sum, col) => sum + df(col.toString()), _ + _)
                  .divide(60d)
                  .as("other")
