...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money, _) => money <= startingMoney * 2d / 3d

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (_, coins) => coins.size <= totalCoins * 2d / 3d

  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold =
    (money, coins) => money * coins.size <= (startingMoney * allCoins.size) / 2d
}
