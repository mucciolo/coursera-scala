...
class ReductionsSuite {
  ...
  @Test def `parallel balance should work`: Unit = {
    def check(input: String, expected: Boolean) =
  ...
  @Test def `parCountChange should invoke the parallel construct 6 times for money == 16, coins == List(1) and moneyThreshold(16)`: Unit = {
