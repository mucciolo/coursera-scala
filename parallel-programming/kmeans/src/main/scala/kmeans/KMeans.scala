...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    var pointByClosestMean = points.groupBy(findClosest(_, means))
    means.foreach(mean => if (!pointByClosestMean.contains(mean)) pointByClosestMean += (mean -> Seq.empty))
    pointByClosestMean
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    var pointByClosestMean = points.groupBy(findClosest(_, means))
    means.foreach(mean => if (!pointByClosestMean.contains(mean)) pointByClosestMean += (mean, ParSeq.empty))
    pointByClosestMean
  }
