...
trait Huffman extends HuffmanInterface {
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    @tailrec
    def iterate(trees: List[CodeTree]): List[CodeTree] =
      if (done(trees)) trees
      else iterate(merge(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def iterate(sub_tree: CodeTree, bits: List[Bit]): List[Char] = sub_tree match {
      case Fork(left, right, _, _) => iterate(if (bits.head == 0) left else right, bits.tail)
      case Leaf(char, _) => if (bits.isEmpty) List(char)
      else char :: iterate(tree, bits)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    @tailrec
    def iterate(table: CodeTable, char: Char): List[Bit] =
      if (table.head._1 == char) table.head._2
      else iterate(table.tail, char)
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    ...
    case Leaf(char, _) => (char, List()) :: Nil
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    a.map{case (char, code) => (char, 0 :: code)} ::: b.map{case (char, code) => (char, 1 :: code)}
