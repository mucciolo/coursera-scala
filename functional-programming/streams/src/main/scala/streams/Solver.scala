...
trait Solver extends GameDef {
   ...
   *
   * Note: it should only return valid neighbors, i.e. block positions
   * that are inside the terrain.
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    LazyList.newBuilder.addAll(
      b.legalNeighbors.map { case (block, move) => (block, move :: history) }
    ).result()
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val initialNeighboors =
      initial.flatMap { case (block, moveList) => newNeighborsOnly(neighborsWithHistory(block, moveList), explored) }

    if (initialNeighboors.isEmpty) {
      initial
    } else {
      initial #::: from(initialNeighboors, explored ++ initialNeighboors.map { case (block, _) => block })
    }
  }
