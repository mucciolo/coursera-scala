...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] = {
    ...
    @tailrec
    def iterate(list: List[Json], acc: List[A]): Option[List[A]] = {

      if (list.isEmpty) {
        Option(acc.reverse)
      } else {
        val maybeA = decoder.decode(list.head)

        if (maybeA.isEmpty)
          Option.empty
        else
          iterate(list.tail, maybeA.get :: acc)
      }
    }

    Decoder.fromFunction {
      case Json.Arr(list) => iterate(list, List.empty)
      case _ => Option.empty
    }
  }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) => fields.find { case (k, _) => k.equals(name) }.map(_._2).flatMap(decoder.decode)
      case _ => Option.empty
    }
...
trait PersonCodecs {
  ...
  private val FIELD_AGE = "age"

  /** The encoder for `Person` */
  implicit val personEncoder: Encoder[Person] =
    ObjectEncoder.field[String](FIELD_NAME)
      .zip(ObjectEncoder.field[Int](FIELD_AGE))
      .transform[Person](user => (user.name, user.age))

  /** The corresponding decoder for `Person` */
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String](FIELD_NAME)
      .zip(Decoder.field[Int](FIELD_AGE))
...
trait ContactsCodecs {
  ...
  private val PEOPLE_FIELD = "people"

  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[List[Person]](PEOPLE_FIELD).transform(contacts => contacts.people)

  implicit val contactsdEcoder: Decoder[Contacts] =
    Decoder.field[List[Person]](PEOPLE_FIELD).transform(people => Contacts(people))
}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maybeJsonArray = parseJson(""" [1, 2, 3] """)
    println(maybeJsonArray.flatMap(_.decodeAs[List[Int]]))
    println(maybeJsonString.flatMap(_.decodeAs[List[Int]]))

    println(Decoder.field[String]("name").decode(maybeJsonObj.get))
    println(Decoder.field[Int]("age").decode(maybeJsonObj.get))
  }
