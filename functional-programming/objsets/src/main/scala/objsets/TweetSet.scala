...
abstract class TweetSet extends TweetSetInterface {
  ...
  def mostRetweetedCarrier(carrier: Tweet): Tweet
}
...
class Empty extends TweetSet {
  ...
  def mostRetweetedCarrier(carrier: Tweet): Tweet = carrier
}

class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {

  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filtered_children = left.filterAcc(p, right.filterAcc(p, acc))

    if (p(this.elem)) filtered_children incl this.elem
    else filtered_children
  }
  ...
  override def union(that: TweetSet): TweetSet = left.union(right.union(that incl this.elem))

  def mostRetweetedCarrier(carrier: Tweet): Tweet = {
    val leftMostRetweeted = left.mostRetweetedCarrier(carrier)
    val rightMostRetweeted = right.mostRetweetedCarrier(carrier)
    val childrenMostRetweeted =
      if (leftMostRetweeted.retweets > rightMostRetweeted.retweets) leftMostRetweeted
      else rightMostRetweeted

    if (childrenMostRetweeted.retweets > this.elem.retweets) childrenMostRetweeted
    else this.elem
  }

  override def mostRetweeted: Tweet = this.mostRetweetedCarrier(new Tweet("", "", -1))
